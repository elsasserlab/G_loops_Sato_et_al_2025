...
bed_peaks_annotated <- impute_feature(bed_peaks, chromhmm_gr, "name")
bed_peaks_annotated_df <- as.data.frame(unique(bed_peaks_annotated))
...
bed_peaks_hmm17 <- impute_feature(bed_peaks, chromhmm17_gr, "name")
# If you want to export to BED file with rtracklayer then you
# do need to explicitly overwrite the name:
bed_peaks_hmm17$name <- bed_peaks_hmm17$feature
bed_peaks_hmm17$score[is.na(bed_peaks_hmm17$score)] <- 0
rtracklayer::export(unique(bed_peaks_hmm17), "../peaks/G4_CnT_combined_peaks_DESeq_annotated_ChromHMM17.bed")
...
bed_peaks_table <- cbind(bed_peaks_annotated_df,data.frame(baseMean=lfc_DKO$baseMean,DHX36=(data_DHX36$isDE & data_DHX36$lfc>1),FANCJ=(data_FANCJ$isDE & data_FANCJ$lfc>1),DKO=(data_DKO$isDE&data_DKO$lfc>1),DHX36lfc=data_DHX36$lfc,FANCJlfc=data_FANCJ$lfc,DKOlfc=data_DKO$lfc))
...
stats <- data.frame(as.data.frame(table(bed_peaks_table$feature))[1],as.data.frame(table(bed_peaks_table$feature[bed_peaks_table$DKO]))[2],as.data.frame(table(bed_peaks_table$feature[!bed_peaks_table$DKO & !bed_peaks_table$DHX36 & !bed_peaks_table$FANCJ]))[2])
