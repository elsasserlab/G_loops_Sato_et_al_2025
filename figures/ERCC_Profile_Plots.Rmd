...
ggscatter(df,"G4_CnT_ERCC_WT_R1","G4_CnT_ERCC_WT_R2") + geom_abline(a=0,b=1,linetype = "dotted")
```

## ERCC KO vs WT
```{r fig.width=4, fig.height=4, error=F,echo=F,prompt=F,warning=F}
df$WT <- (df$G4_CnT_ERCC_WT_R1+df$G4_CnT_ERCC_WT_R2)/2
df$KO <- (df$G4_CnT_ERCC_KO_R1+df$G4_CnT_ERCC_KO_R2)/2
...
ggscatter(rdf[rdf$Rloop_CnT_ERCC_WT_R1<500,],"Rloop_CnT_ERCC_WT_R1","Rloop_CnT_ERCC_WT_R2") + geom_abline(a=0,b=1,linetype = "dotted")
```

## ERCC KO vs WT
```{r fig.width=4, fig.height=4, error=F,echo=F,prompt=F,warning=F}
rdf$WT <- (rdf$Rloop_CnT_ERCC_WT_R1+rdf$Rloop_CnT_ERCC_WT_R2)/2
rdf$KO <- (rdf$Rloop_CnT_ERCC_KO_R1+rdf$Rloop_CnT_ERCC_KO_R2)/2
...
h2 <- plot_bw_hesort_vh2 <- plot_bw_heatmap(G4.BWs[1],"../peaks/G4_CnT_combined_peaks_DJ_nonDJ.bed",mode="center",order_by=sort_o,max_rows_allowed = 200,verbose=F, zmax = 21)
...
ggarrange(h1,h2,h3,h4, ncol = 4,common.legend = T)
```
